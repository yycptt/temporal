...
func UpdateComponent[C Component, I any, O any](
	...
	ref, err := engineFromContext(ctx).updateComponent(
		...
		func(ctx MutableContext, c Component) error {
			var err error
			// TODO: return error if c.(C) fails.
			// The C here can be a different type than the registered component type.
			// e.g. this can be an interface type, while the registered type must be an interface
