...
func (c *clientImpl) ClosePayloadStore(
	...
	shardID := c.shardIDFromWorkflowID(request.GetNamespaceId(), request.GetStoreId())
	var response *historyservice.ClosePayloadStoreResponse
	op := func(ctx context.Context, client historyservice.HistoryServiceClient) error {
		var err error
		ctx, cancel := c.createContext(ctx)
		defer cancel()
		response, err = client.ClosePayloadStore(ctx, request, opts...)
		return err
	}
	if err := c.executeWithRedirect(ctx, shardID, op); err != nil {
		return nil, err
...
func (c *clientImpl) RecordActivityTaskHeartbeat(
	...
	shardID := c.shardIDFromWorkflowID(request.GetNamespaceId(), taskToken.GetWorkflowId())
